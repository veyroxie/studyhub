...
| B9 | **SHIPPED 2026-08-27** — Nadine wants both after all: cancel+credit stays for per-student make-ups; reschedule moves one dated session wholesale (class_session_moves, migration 0042; announcement on move and undo; no credits; 409 on cancelled sessions; calendar week/month/day-modal + attendance day list + iCal same-UID relocation; locked by TestSessionMove_FullLifecycle and the iCal move assertion). Earlier: **PARKED 2026-08-27: Nadine prefers cancel+credit** ("some students can't make it for the same replacement date") — await her final word on wanting both, otherwise drop. Was: Reschedule a class session (decided 2026-08-26): alongside cancel+credit (per-student make-ups at different times), admin can move ONE dated session wholesale to another date for all students. Shape: a `class_session_moves` exception row (tenant_id, class_id, from_date, to_date) in the 0040 keyed-by-(class_id,date) family — NOT a cancellation (no credits granted: the class still happens). Touches: calendar render (strike original, show moved), the 8.7 session expander (classify dates held/cancelled/holiday/moved-out/moved-in; cross-month moves change billed counts), iCal (same UID, new DTSTART), and parent announcement on move. Build with/after the session expander so billing and display agree. | MED | M |
| A17 | **(a) and (c) SHIPPED 2026-08-27** (editable line names; attendance Undo + DELETE endpoint, locked by TestAttendance_UndoDeletesRow). Remaining: (b) invoice-number editing — decide build vs tell her it is system-assigned. Original: Promised to Nadine but never delivered (found in the 27/08 full audit of her asks): (a) subject-named invoice lines — Math/Mandarin/Phonics/30-min options, asked 07-13/08, Ely acked "will update when all donee", then silently deferred into v2; the builder still offers only Group/Private-by-level and line NAMES are not editable inline (price/qty are). Interim fix = make the line name editable; real fix = 8.7 lines carry the class name. (b) invoice-number editing, asked 31/07, promised "tonight", never shipped — decide: build it or tell her the number is system-assigned. (c) attendance undo, asked 13/08 with video — no undo path exists in attendance.js. TELL NADINE which of these are coming vs not — she was told yes on all three. | HIGH | M |
| A18 | Generate invoices for an arbitrary month (Nadine 07/08): the manual cron only runs the current month. 0039's period dedup makes this safe to build (R5); becomes trivial after 8.7. | MED | S |
...
- NEW-31 CLOSED 2026-09-01: `store.SessionRateOn` prices each session at the
  duration it actually ran (resolved through 0047's schedule versions), and the
  iCal feed stamps historical times. Original finding:
