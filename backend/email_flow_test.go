...
func setupEmailTestApp(t *testing.T) (*chi.Mux, *DB, func()) {
	t.Helper()
	dsn := testDSN()
	db := initDB(dsn)
...
func TestVerifyEmail_Parent_ActivatesAndLogsIn(t *testing.T) {
	...
	if err := db.QueryRow(
		...
		"verify@example.com", tokenPurposeVerifyParent,
	).Scan(&token); err != nil {
...
func TestRegisterTeacher_CreatesPendingRegistration(t *testing.T) {
	...
	var tokenCount int
	db.QueryRow(`SELECT COUNT(*) FROM email_tokens WHERE email=? AND purpose=?`, "teacher@example.com", tokenPurposeVerifyTeacher).Scan(&tokenCount)
	if tokenCount == 0 {
...
func TestVerifyEmail_Teacher_MarksRegistrationVerified_NoLogin(t *testing.T) {
	...
	var token string
	db.QueryRow(`SELECT token FROM email_tokens WHERE email=? AND purpose=?`, "vteacher@example.com", tokenPurposeVerifyTeacher).Scan(&token)
...
func TestResetPassword_HappyPath(t *testing.T) {
	...
	var token string
	db.QueryRow(`SELECT token FROM email_tokens WHERE email=? AND purpose=?`, "admin@studyhub.com", tokenPurposeResetPassword).Scan(&token)
	if token == "" {
...
func TestSetPassword_HappyPath(t *testing.T) {
	...
	hash, _ := hashPassword("placeholder-discarded")
	var userID int64
	db.QueryRow(
	...
	tok, err := createEmailToken(db, "newteacher@example.com", tokenPurposeSetPassword, &userID, nil, setPasswordTokenTTL)
	if err != nil {
