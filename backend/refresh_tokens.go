...
func handleRefresh(db *DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil || cookie.Value == "" {
			respondError(w, "missing refresh token", http.StatusUnauthorized)
			return
		}
		...
		if usedAt != nil || revokedAt != nil {
			...
			logAudit(db, "system", "refresh_token_reused", "user", "", "family="+family+" — possible theft, family revoked")
			respondError(w, "session terminated — please sign in again", http.StatusUnauthorized)
			return
		}
		if time.Now().After(expiresAt) {
			respondError(w, "refresh token expired", http.StatusUnauthorized)
			return
		}
		...
		if err := db.QueryRow(
			...
			respondError(w, "user no longer exists", http.StatusUnauthorized)
			return
		}
		if status != "active" {
			db.Exec(`UPDATE refresh_tokens SET revoked_at=NOW() WHERE token_family=? AND revoked_at IS NULL`, family)
			respondError(w, "account is "+status, http.StatusForbidden)
			return
		}
		if _, err := db.Exec(`UPDATE refresh_tokens SET used_at=NOW() WHERE token_hash=?`, hash); err != nil {
			respondError(w, "could not rotate token", http.StatusInternalServerError)
			return
		}
		...
		if err := issueRefreshToken(db, w, r, userID, tenantID, family); err != nil {
			respondError(w, "could not issue refresh token", http.StatusInternalServerError)
			return
		}
		...
		issueAuthCookie(w, r, userID, tenantID, email, role, name, false)
		respond(w, map[string]string{"status": "rotated"})
	}
}
...
func revokeRefreshFamilyByUser(db *DB, userID int, reason string) {
	...
	if err != nil {
		logger.Error("revoke refresh family failed", "err", err, "user_id", userID)
		return
	}
	logAudit(db, "system", "refresh_family_revoked", "user", "", reason)
}
