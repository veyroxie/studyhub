...
func processEmailQueue(db *DB) int {
	...
	if err != nil {
		logger.Error("email_queue: select pending failed", "err", err)
		return 0
	}
	type job struct {
		id       int64
	...
	count := 0
	for _, j := range jobs {
		err := mailer.Send(j.to, j.subject, j.body)
		count++
		if err == nil {
		...
		if nextAttempts >= emailMaxAttempts {
			...
			logger.Error("email_queue: gave up", "id", j.id, "to", j.to, "err", err)
			continue
		}
		...
		logger.Warn("email_queue: send failed, will retry", "id", j.id, "attempts", nextAttempts, "err", err)
	}
	return count
}
...
func purgeOldEmailQueueRows(db *DB) {
	...
	if err != nil {
		logger.Error("email_queue: purge failed", "err", err)
		return
	}
	if n, _ := res.RowsAffected(); n > 0 {
		logger.Info("email_queue: pruned", "count", n)
	}
}
