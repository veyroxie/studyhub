...
func InitPush() {
	...
	if !isPushConfigured() {
		core.Logger.Warn("web push disabled — VAPID_PUBLIC_KEY / VAPID_PRIVATE_KEY not set; parents get email + in-app only")
		return
	}
	core.Logger.Info("web push initialised")
}
...
func sendPushToParent(db *store.DB, tenantID int, parentEmail string, payload pushPayload) {
	if !isPushConfigured() {
		return
	}
	body, err := json.Marshal(payload)
	if err != nil {
		core.Logger.Error("push payload marshal failed", "err", err)
		return
	}
...
func listParentSubs(db *store.DB, tenantID int, parentEmail string) []webpush.Subscription {
	...
	if err != nil {
		core.Logger.Error("push subscription lookup failed", "err", err, "email", parentEmail)
		return nil
	}
	defer rows.Close()
...
func deliverPush(db *store.DB, tenantID int, sub *webpush.Subscription, body []byte) {
	...
	if err != nil {
		core.Logger.Error("push send failed", "err", err, "endpoint", sub.Endpoint)
		return
	}
	...
	if resp.StatusCode == http.StatusNotFound || resp.StatusCode == http.StatusGone {
		if _, derr := db.Exec(`DELETE FROM push_subscriptions WHERE tenant_id=? AND endpoint=?`, tenantID, sub.Endpoint); derr != nil {
			core.Logger.Error("push subscription prune failed", "err", derr, "endpoint", sub.Endpoint)
		}
		return
	}
	if resp.StatusCode >= 300 {
		core.Logger.Warn("push service rejected", "status", resp.StatusCode, "endpoint", sub.Endpoint)
	}
}

// VapidPublicKey returns the configured VAPID public key (safe to expose to
// browsers). Empty when web push is not configured.
func VapidPublicKey() string { return pushCfg.publicKey }
