...
func NotifyParentOnCheck(db *store.DB, tenantID int, a models.Attendance, isCheckIn bool) {
	verb := "checked out"
...
func NotifySelfStudyCheckIn(db *store.DB, tenantID int, studentID, startTime string) {
...
func dispatchParentNotify(db *store.DB, tenantID int, studentID, verb, context, eventTime string) {
...
func lookupStudentForNotify(db *store.DB, tenantID int, studentID string) (studentNotify, bool) {
	var s studentNotify
...
func classNameByID(db *store.DB, tenantID int, classID *string) string {
...
func maybeEmailParent(db *store.DB, parentEmail, parentName, subject, detail string) {
	var wantsEmail bool
	db.QueryRow(`SELECT COALESCE(notify_checkin_email,false) FROM users WHERE email=?`, parentEmail).Scan(&wantsEmail)
	if !wantsEmail {
		return
	}
	if err := core.SendEmail(parentEmail, subject, mailer.RenderCheckEventEmail(parentName, subject, detail)); err != nil {
		core.Logger.Error("check-in email failed", "err", err, "email", parentEmail)
	}
}
