...
func HandleDSARExport(db *store.DB) http.HandlerFunc {
...
func dsarParentFamily(db *store.DB, c *core.Claims) any {
...
func dsarParentStudents(db *store.DB, c *core.Claims) any {
	return listStudents(db, c)
}

func dsarParentProgressReports(db *store.DB, c *core.Claims) any {
...
func dsarTeacherSelf(db *store.DB, c *core.Claims) any {
	tw, twArgs := store.ScopeTenant(c, "")
	args := append([]any{c.Email}, twArgs...)
...
func dsarTeacherPayroll(db *store.DB, c *core.Claims) any {
	tw, twArgs := store.ScopeTenant(c, "")
	args := append([]any{c.Email}, twArgs...)
	var staffID string
