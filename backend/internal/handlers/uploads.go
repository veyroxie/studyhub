...
func InitUploads() {
	if os.Getenv("UPLOAD_STORE") != "s3" {
		core.Logger.Info("uploads: local disk")
		return
	}
	cfg := s3Config{
	...
	if cfg.endpoint == "" || cfg.bucket == "" || cfg.accessKey == "" || cfg.secretKey == "" {
		core.Logger.Warn("UPLOAD_STORE=s3 but endpoint/bucket/key not set; falling back to local")
		return
	}
	if cfg.region == "" {
	...
	core.Logger.Info("uploads: s3", "endpoint", cfg.endpoint, "bucket", cfg.bucket)
}
