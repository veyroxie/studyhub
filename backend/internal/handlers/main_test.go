...
func setupTestApp(t *testing.T) (*chi.Mux, func()) {
	...
	r.Group(func(r chi.Router) {
		...
		r.Delete("/api/invoices/{id}", HandleInvoiceDelete(db))
...
func TestStudents_Create_Parent_Forbidden(t *testing.T) {
	...
	defer cleanup()
	token := getParentToken(t, r)
	w := doRequest(r, "POST", "/api/students", token, models.Student{FirstName: "Hack"})
...
func TestClasses_Create(t *testing.T) {
	...
	defer cleanup()
	token := getAdminToken(t, r)
	cls := models.Class{Name: "Test Class", Day: "Wednesday", Time: "10:00", EndTime: "11:00", Capacity: 5, Color: "green"}
...
func TestClasses_TeacherIDs_RoundTrip(t *testing.T) {
	...
	defer cleanup()
	token := getAdminToken(t, r)
	cls := models.Class{Name: "Multi Teacher", Day: "Friday", Time: "14:00", EndTime: "15:00", Capacity: 6, TeacherIDs: []string{"s1", "s2"}}
	w := doRequest(r, "POST", "/api/classes", token, cls)
...
func TestInvoices_Create_And_MarkPaid(t *testing.T) {
	...
	defer cleanup()
	token := getAdminToken(t, r)
	inv := models.Invoice{StudentID: "STU001", Description: "Test Invoice", Type: "Adhoc", Amount: 99.50, DueDate: "2026-04-30"}
...
func TestInvoices_NegativeAmount_Rejected(t *testing.T) {
	...
	defer cleanup()
	token := getAdminToken(t, r)
	inv := models.Invoice{StudentID: "STU001", Description: "Bad Invoice", Amount: -50}
	w := doRequest(r, "POST", "/api/invoices", token, inv)
	if w.Code == http.StatusOK {
...
func TestAnnouncements_Create_Parent_Forbidden(t *testing.T) {
	...
	defer cleanup()
	token := getParentToken(t, r)
	w := doRequest(r, "POST", "/api/announcements", token, models.Announcement{Title: "Hack", Message: "...", Type: "Notice"})
	if w.Code != http.StatusForbidden {
		t.Fatalf("expected 403 got %d", w.Code)
	}
}
...
func TestAttendance_CheckIn_Then_CheckOut(t *testing.T) {
	...
	att := models.Attendance{PersonID: "STU002", PersonType: "student", Date: "2026-03-10", ClassID: &classID, CheckIn: &checkIn, Status: "Present"}
...
func TestStaff_Create_Admin(t *testing.T) {
	...
	defer cleanup()
	token := getAdminToken(t, r)
	s := models.Staff{Name: "New", FullName: "Teacher New", Role: "Teacher", Email: "new@test.com", Salary: 3000, JoinDate: "2026-03-01", Status: "Active"}
...
func TestXSS_InStudentName(t *testing.T) {
	...
	defer cleanup()
	token := getAdminToken(t, r)
	s := models.Student{FirstName: "<script>alert(1)</script>", LastName: "Test", Contact: "xss@test.com", Status: "Active", EnrolledClasses: []string{}, Siblings: []string{}}
