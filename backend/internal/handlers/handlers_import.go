...
func HandleImport(db *store.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, p := range parents {
			...
			if _, err := db.Exec(
				...
				famID, tid, familyName, p.email, p.phone, p.name, core.NewReferralCode(),
			); err != nil {
		...
		for _, p := range parents {
			...
			token, err := store.CreateEmailToken(db, p.email, store.TokenPurposeSetPassword, &p.userID, nil, store.VerifyTokenTTL)
			if err != nil {
			...
			if err := core.SendEmail(p.email, "Welcome to The Study Hub — set your password", mailer.RenderParentWelcomeEmail(p.name, setURL)); err != nil {
...
func HandleClearSeedData(db *store.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		c := core.ClaimsFrom(r)
		l := core.LogFromReq(r)
