...
func listClasses(db *store.DB, c *core.Claims) []models.Class {
	tw, twArgs := store.ScopeTenant(c, "")
	rows, err := db.Query(`SELECT id,name,teacher_ids,classroom,day,time,end_time,capacity,enrolled,color,category,COALESCE(class_type,'Group'),COALESCE(level_band,''),COALESCE(subject,'') FROM classes WHERE deleted_at IS NULL`+tw, twArgs...)
	if err != nil {
...
func HandleClasses(db *store.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPost:
			...
			if _, err := db.Exec(`INSERT INTO classes(id,tenant_id,name,teacher_ids,classroom,day,time,end_time,capacity,enrolled,color,category,class_type,level_band,subject) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
				c.ID, tid, c.Name, models.JSONArr(c.TeacherIDs), c.Classroom, c.Day, c.Time, c.EndTime, c.Capacity, c.Enrolled, c.Color, c.Category, c.ClassType, c.LevelBand, c.Subject); err != nil {
...
func HandleClassByID(db *store.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPut:
			...
			args := append([]any{cl.Name, models.JSONArr(cl.TeacherIDs), cl.Classroom, cl.Day, cl.Time, cl.EndTime, cl.Capacity, cl.Color, cl.Category, cl.ClassType, cl.LevelBand, cl.Subject, id}, twArgs...)
			res, err := db.Exec(`UPDATE classes SET name=?,teacher_ids=?,classroom=?,day=?,time=?,end_time=?,capacity=?,color=?,category=?,class_type=?,level_band=?,subject=? WHERE id=?`+tw+` AND deleted_at IS NULL`, args...)
			if err != nil {
