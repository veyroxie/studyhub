...
// Init wires up the process-wide email sender at startup and registers it with
// core so any layer can send via core.SendEmail. In production it talks to
// Resend's HTTP API; in dev (no RESEND_API_KEY set) it logs the message to
// stdout so developers can copy verification links straight from the terminal.
//
...
func Init() {
	...
	core.SetMailer(&resendMailer{apiKey: apiKey, from: from, client: &http.Client{Timeout: 10 * time.Second}})
	core.Logger.Info("mailer initialised", "provider", "resend", "from", from)
}
...
func (m *resendMailer) Send(to, subject, htmlBody string) error {
	...
	core.Logger.Info("email sent", "to", to, "subject", subject)
	return nil
}
...
func (m *devMailer) Send(to, subject, htmlBody string) error {
	...
	core.Logger.Info("MAILER[DEV] would send email",
		"from", m.from,
		"to", to,
		"subject", subject,
		"body", htmlBody,
...
var emailBrand = store.DefaultTenantSettings
var emailBrandDB *store.DB

// InitBrand wires the DB handle so Brand() can fetch live values.
// Still pre-loads tenant 1 at boot so the very first email render after
// startup is warm.
func InitBrand(db *store.DB) {
	emailBrandDB = db
	emailBrand = store.LoadTenantSettings(db, 1)
}
...
func Brand() *store.TenantSettings {
	if emailBrandDB == nil {
		return emailBrand
	}
	return store.LoadTenantSettings(emailBrandDB, 1)
}
