...
func HandleMFASetup(db *store.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if enabled {
			core.RespondError(w, "MFA already enabled — disable first to re-enrol", http.StatusConflict)
			return
		}
		...
		if _, err := db.Exec(`UPDATE users SET mfa_secret=? WHERE id=?`, secret, c.UserID); err != nil {
			core.RespondError(w, "could not store secret", 500)
			return
		}
...
func HandleMFAConfirm(db *store.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		c := core.ClaimsFrom(r)
		if !core.IsAdminRole(c) {
			core.RespondError(w, "admin only", http.StatusForbidden)
		...
		if secret == "" {
			core.RespondError(w, "no MFA setup in progress — start with POST /api/auth/mfa/setup", http.StatusBadRequest)
			return
		}
		if enabled {
			core.RespondError(w, "MFA already confirmed", http.StatusConflict)
			return
		}
		if !verifyTOTPCode(secret, body.Code) {
			core.RespondError(w, "code did not match — check your authenticator clock", http.StatusBadRequest)
			return
		}
		...
		codesJSON := models.JSONArr(hashed)
...
func HandleMFADisable(db *store.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		c := core.ClaimsFrom(r)
		if !core.IsAdminRole(c) {
			core.RespondError(w, "admin only", http.StatusForbidden)
...
func HandleMFAVerify(db *store.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var body struct {
		...
		if err != nil {
			core.RespondError(w, "intermediate token is invalid or expired — start login again", http.StatusUnauthorized)
			return
		}
...
func consumeRecoveryCode(db *store.DB, userID int, supplied, recoveryJSON string) bool {
	hashes := models.ParseArr(recoveryJSON)
	...
	db.Exec(`UPDATE users SET mfa_recovery_codes=? WHERE id=?`, models.JSONArr(remaining), userID)
	return true
}
...
func consumeMFAIntermediate(db *store.DB, token string) (*mfaIntermediate, error) {
	if token == "" {
