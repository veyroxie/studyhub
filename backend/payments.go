...
func handlePaymentCheckout(db *DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := db.QueryRow(`SELECT student_id, description, amount FROM invoices WHERE id=? AND deleted_at IS NULL AND status<>'Paid'`+tw, readArgs...).Scan(&studentID, &description, &amount); err != nil {
			respondError(w, "invoice not found or already paid", http.StatusNotFound)
			return
		}
		if c.Role == "parent" {
		...
		default:
			respondError(w, "unknown payment provider", http.StatusBadRequest)
			return
		}
		if err != nil {
			logger.Error("checkout create failed", "err", err, "provider", provider, "invoice_id", id)
			...
			respondError(w, msg, http.StatusBadGateway)
			return
		}
		logAudit(db, c.Email, "payment_checkout_created", "invoice", id, provider)
		respond(w, map[string]string{"url": checkoutURL, "provider": provider})
	}
}
...
func createBillplzBill(invoiceID, email, name, description string, amount float64) (string, error) {
	...
	form.Set("redirect_url", appURL()+"/#billing?paid="+invoiceID)
	form.Set("callback_url", appURL()+"/api/payments/webhook/billplz")
...
func handleBillplzWebhook(db *DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if secret != "" {
			...
			if !verifyBillplzSignature(r.Form, secret, given) {
				respondError(w, "invalid signature", http.StatusForbidden)
				return
			}
		}
		...
		if err != nil {
			logger.Error("billplz webhook update failed", "err", err, "invoice_id", invoiceID)
			respondError(w, "db error", 500)
			return
		}
		if n, _ := res.RowsAffected(); n > 0 {
			logger.Info("billplz webhook: invoice marked paid", "invoice_id", invoiceID, "bill_id", billID)
			logAudit(db, "billplz", "invoice_paid", "invoice", invoiceID, "via webhook")
			var studentID string
			db.QueryRow(`SELECT student_id FROM invoices WHERE id=?`, invoiceID).Scan(&studentID)
			if studentID != "" {
				referralCheckMilestoneOnPay(db, studentID, nil)
			}
		}
		w.WriteHeader(http.StatusOK)
	}
...
func createStripeCheckout(invoiceID, email, description string, amount float64) (string, error) {
	...
	form.Set("success_url", appURL()+"/#billing?paid="+invoiceID)
	form.Set("cancel_url", appURL()+"/#billing")
...
func handleStripeWebhook(db *DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Read body raw — signature is computed over the exact bytes.
		body, err := io.ReadAll(io.LimitReader(r.Body, 1<<20))
		if err != nil {
			respondError(w, "read failed", 400)
			return
		}
		...
		if err != nil {
			logger.Error("stripe webhook update failed", "err", err, "invoice_id", invoiceID)
			respondError(w, "db error", 500)
			return
		}
		if n, _ := res.RowsAffected(); n > 0 {
			logger.Info("stripe webhook: invoice marked paid", "invoice_id", invoiceID, "session_id", evt.Data.Object.ID)
			logAudit(db, "stripe", "invoice_paid", "invoice", invoiceID, "via webhook")
			var studentID string
			db.QueryRow(`SELECT student_id FROM invoices WHERE id=?`, invoiceID).Scan(&studentID)
			if studentID != "" {
				referralCheckMilestoneOnPay(db, studentID, nil)
