...
class ReportRequest(BaseModel):
    ...
    reportType: str = "summary"  # "summary" | "detailed" | "risk"
...
def generate_report(req: ReportRequest):
    ...
    elif req.reportType == "risk":
        data = _risk_report(req)
    elif req.reportType == "detailed":
...
def _summary_report(req: ReportRequest) -> dict:
    """High-level KPIs: active students, revenue, avg attendance."""
    active = sum(1 for s in req.students if s.status == "Active")
    total_revenue = sum(i.amount for i in req.invoices)
    collected = sum(i.amount for i in req.invoices if i.status == "Paid")
    ...
    present = sum(1 for a in student_att if a.status in ("Present", "Late"))
...
def _risk_report(req: ReportRequest) -> dict:
    """
    ...
    for s in req.students:
        ...
        overdue = [i for i in req.invoices if i.studentId == s.id and i.status == "Overdue"]
        if overdue:
...
def _detailed_report(req: ReportRequest) -> dict:
    ...
    for s in req.students:
        ...
        unpaid = sum(i.amount for i in invoices if i.status != "Paid")
