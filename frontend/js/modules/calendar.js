(function() {
  ...
  function render(container) {
    ...
    const viewToggle = isClient ? '' : '<div style="display:flex;gap:0.25rem;background:#f1f5f9;border-radius:4px;padding:3px">'
      ...
      + (isAdmin ? '<button onclick="App.Calendar._setView(\'programs\')" style="padding:0.3rem 0.85rem;font-size:0.72rem;font-weight:600;border:none;border-radius:4px;cursor:pointer;background:' + (_view==='programs'?'var(--gold, #f59e0b)':'transparent') + ';color:' + (_view==='programs'?'#0a0a0a':'#94a3b8') + '">Programs</button>' : '')
      + '</div>';
