...
.DEFAULT_GOAL := help
.PHONY: help check check-full test-up test-dev test-down test-reset release deploy deploy-nobuild verify ship logs psql ssh failed-emails migration-dryrun backup-pull backup-verify-remote
...
backup-verify-remote:  ## download the newest off-site backup and prove it restores
	./scripts/backup-verify-remote.sh

backup-pull:  ## copy the droplet's database backups to this machine (free off-site copy)
